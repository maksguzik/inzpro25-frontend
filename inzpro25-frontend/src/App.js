...
function App() {
  ...
  const [isGetUserRoleFunctionCalled, setIsGetUserRoleFunctionCalled] =
    useState(false);
  ...
  <Routes>
    ...
    <Route
      path="/"
      element={<RootLayout isAdmin={roles.includes('ADMIN')} isUser={roles.includes('USER')} isSupportTeam={roles.includes('SUPPORT_TEAM')} />}
    >
      {/* Default Redirect Based on Role */}
      <Route
        index
        element={
          roles.includes('ADMIN') && roles.includes('SUPPORT_TEAM') && roles.includes('USER') ? (
            <Navigate to="/home/dashboard" replace />
          ) : roles.includes('ADMIN') ? (
            <Navigate to="/device-management" replace />
          ) : roles.includes('SUPPORT_TEAM') ? (
            <Navigate to="/home/dashboard" replace />
          ) : roles.includes('USER') ? (
            <Navigate to="/my-devices" replace />
          ) : (
            <HomePage />
          )
        }
