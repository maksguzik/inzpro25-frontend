...
function CompanyList() {
  ...
  const getCompanyListOrderedByName = async () => {
    const token = await getAccessTokenSilently();
    fetch(
      `${URL}api/companies?name=${searchName}&page=${currentPage}&size=5&sortBy=${sortBy}&order=${order}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    <>
      <div className="orderAddDeleteContainer">
        ...
        <div className="deleteAddContainer">
          <AddCompany setUpdateCompanyList={setUpdateCompanyList} />
      ...
      <div className="companyListContainer">
        {companyList.length > 0 ? (
          ...
          <div className="no-results-message">No companies found.</div>
        )}
      </div>
    </>
  );
